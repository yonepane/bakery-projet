"""Financial report computation helpers — pure DB aggregation & CSV utilities.

Extracted from routers/reports.py and routers/finance.py so that the router
layer stays focused on HTTP concerns (validation, auth, response wrapping).
"""
import csv
import io
...
def _to_csv_response_rows(header: list, rows: list, report_name: str) -> tuple:
    """Build the CSV content and content-disposition header for a report.

    Returns (content_str, filename, media_type) so the caller can wrap
    ...
    output.seek(0)
    ts = _now_fn().strftime("%Y%m%d_%H%M")
    return output.getvalue(), f"{report_name}_{ts}.csv", "text/csv"


# ── Semi-finished cost helper ───────────────────────────────────────────────────
...
def compute_production_margin(
    ...
    return {
        ...
        "calculated_at": _now_fn().isoformat(),
    }


# ── Supplier Ledger ─────────────────────────────────────────────────────────────

def compute_supplier_ledger(db: sqlalchemy.orm.Session, owner_id: int) -> dict:
    suppliers = (
...
def build_stock_valuation_csv_rows(db: sqlalchemy.orm.Session, owner_id: int) -> tuple:
    """Return (header, rows) for stock-valuation CSV export."""
    val = compute_stock_valuation(db, owner_id)
...
def build_production_margin_csv_rows(
    db: sqlalchemy.orm.Session, owner_id: int, limit: int = 200
) -> tuple:
    raw = compute_production_margin(db, owner_id, limit=limit)
...
def build_supplier_ledger_csv_rows(db: sqlalchemy.orm.Session, owner_id: int) -> tuple:
    raw = compute_supplier_ledger(db, owner_id)
