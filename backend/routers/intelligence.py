...
async def get_production_suggestions(
    ...
    forecast_resp = await get_enhanced_forecast(target_date, horizon_days=1, db=db, owner_id=owner_id)
...
async def get_purchase_suggestions(
    ...
    prod_resp = await get_production_suggestions(target_date, db=db, owner_id=owner_id)
...
async def get_expiring_stock_usage_suggestions(
    db: sqlalchemy.orm.Session = Depends(get_db),
    owner_id: int = Depends(get_effective_owner_id),
