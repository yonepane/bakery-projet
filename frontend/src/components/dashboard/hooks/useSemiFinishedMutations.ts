...
export function useSemiFinishedMutations({ fetchTabData, addToast }: MutationDeps) {
  const { execute: handleCreateSemiFinished } = useMutation(
    async (payload: { name: string; unit: string; min_threshold: number; shelf_life_hours?: number | null; allergens?: string[] | null; }) => api.post('/api/semi-finished', payload),
  ...
  const { execute: handleSaveRecipe } = useMutation(
    async (itemId: number, items: Array<{ ingredient_id: number; quantity: number }>) => api.put(`/api/semi-finished/${itemId}/recipe`, { items }),
  ...
  const { execute: handleDeleteSemiFinished } = useMutation(
    async (itemId: number) => api.delete(`/api/semi-finished/${itemId}`),
